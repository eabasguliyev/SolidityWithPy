from brownie import (accounts, network, config,
 Contract, MockV3Aggregator, VRFCoordinatorMock, LinkToken, interface)
...
#Check chainlink github
def get_contract(contract_name):
    """This function will grab the contract addresses from the brownie config
    if defined, otherwise, it will deploy a mock version of that cotnract and return that
    mock contract.
    Args:
...
def deploy_mock(contract_name, decimals = DECIMALS, initial_value = INITIAL_VALUE):
    account = get_account()
    match contract_name:
